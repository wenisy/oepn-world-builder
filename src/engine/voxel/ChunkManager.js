...
export class ChunkManager {
  ...
  generateChunk(chunkX, chunkY, chunkZ) {
    console.log(`生成区块: (${chunkX}, ${chunkY}, ${chunkZ})`);
    // 创建区块数据
  ...
  generateHeight(x, z) {
    ...
    noise += Math.sin(x * 0.05) * Math.cos(z * 0.05) * 10;
    noise += Math.sin(x * 0.02) * Math.cos(z * 0.02) * 15;

    // 生成山峰
    const distance = Math.sqrt(x * x + z * z);
    if (distance > 100) {
      noise -= (distance - 100) * 0.1;
    }

    const height = Math.floor(baseHeight + noise);

    // 打印调试信息
    if (x % 32 === 0 && z % 32 === 0) {
      console.log(`生成高度: (${x}, ${z}) => ${height}`);
    }
